...
use super::{RestartPolicy, ServiceConfig, ServiceType};
...
impl ServiceParser {
            ...
            if let Some(eq_pos) = line.find('=') {
                ...
                match key {
                    ...
                    "ExecStop" => {
                        ...
                        stop_cmd
                            .push_str(value)
                            .map_err(|_| "Stop command too long")?;
                    ...
                    _ => {
                        ...
                        if let Some(env_key) = key.strip_prefix("ENV_") {
    ...
    pub fn generate_service_file(config: &ServiceConfig) -> Result<String<2048>, &'static str> {
        ...
        // Header comment
        content
            .push_str("# Mach_R Service Configuration\n")
            .map_err(|_| "Content too long")?;
        content
            .push_str("# Generated by Mach_R Init System\n\n")
            .map_err(|_| "Content too long")?;

        // Service name
        content.push_str("Name=").map_err(|_| "Content too long")?;
        content
            .push_str(&config.name)
            .map_err(|_| "Content too long")?;
        content.push('\n').map_err(|_| "Content too long")?;
        ...
        if !config.working_directory.is_empty() {
            content
                .push_str("WorkingDirectory=")
                .map_err(|_| "Content too long")?;
            content
                .push_str(&config.working_directory)
                .map_err(|_| "Content too long")?;
            content.push('\n').map_err(|_| "Content too long")?;
        }
        ...
        if config.restart_delay_ms != 1000 {
            ...
            if seconds < 10 {
                content
                    .push((b'0' + seconds as u8) as char)
                    .map_err(|_| "Content too long")?;
            } else {
    ...
    pub fn validate_config(config: &ServiceConfig) -> Result<Vec<String<128>, 16>, &'static str> {
        ...
        if config.working_directory.is_empty() {
            ...
            warning
                .push_str("No working directory specified, will use /")
        ...
        if config.user.is_empty() || config.user == "root" {
            let mut warning = String::new();
            warning
                .push_str("Service will run as root, consider using a dedicated user")
                .map_err(|_| "Warning too long")?;
            warnings.push(warning).map_err(|_| "Too many warnings")?;
        }
        ...
        if config.restart == RestartPolicy::Always && config.service_type == ServiceType::Oneshot {
            let mut warning = String::new();
            warning
                .push_str("Always restart with oneshot type may cause rapid restarts")
                .map_err(|_| "Warning too long")?;
            warnings.push(warning).map_err(|_| "Too many warnings")?;
        }
...
impl ServiceTemplates {
    ...
    pub fn network_service(
        ...
        command: &str,
        _port: u16,
    ) -> Result<ServiceConfig, &'static str> {
        let mut config = ServiceConfig::new(name)?;
        config.set_exec_start(command)?;
        config.service_type = ServiceType::Simple;
