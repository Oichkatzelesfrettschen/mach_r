...
// Allow `<< 0` for hardware register field documentation clarity
#![allow(clippy::identity_op)]
// Page tables have complex initialization, Default derives are not appropriate
...
impl PageTableManager {
    ...
    pub fn allocate_table(
        &mut self,
        allocate_fn: impl FnOnce() -> Option<&'static mut PageTable>,
    ...
    pub fn map_page(
        ...
        for &vpn_idx in vpn.iter().take(3) {
            let entry = unsafe { (*current_table).get_entry(vpn_idx) };

            if !entry.is_valid() {
                // Need to create a new table
                let new_table_ptr = self
                    .allocate_table(&allocate_fn)
                ...
                unsafe {
                    (*current_table).set_entry(vpn_idx, new_entry);
                }
...
pub fn init_mmu(page_table_addr: u64) {
    unsafe {
        ...
        let mair_value = (MAIR_DEVICE_NGNRNE << 0) |   // Index 0: Device memory
        ...
        let tcr_value = (16u64 << 0) |     // T0SZ: 48-bit virtual addresses for TTBR0_EL1
