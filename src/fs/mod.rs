...
impl FilesystemManager {
    ...
    /// Mount a filesystem
    pub fn mount(&mut self, path: &str, device: &str, fs_type: FilesystemType) -> Result<(), &'static str> {
        if !self.initialized {
        ...
        mount_device.push_str(device).map_err(|_| "Device path too long")?;
        ...
        self.mounts.push(mount_point).map_err(|_| "Too many mount points")?;
        Ok(())
    }
