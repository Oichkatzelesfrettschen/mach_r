...
impl RustStubGenerator {
    ...
    fn generate_header(&self, analyzed: &AnalyzedSubsystem) -> Result<String, CodegenError> {
        ...
        output.push_str(&format!(
            "//! Generated by mig-rust from {}.defs\n",
            analyzed.name
        ));
        output.push_str("//!\n");
    ...
    fn generate_message_types(
        &self,
        routine: &AnalyzedRoutine,
        _subsystem: &AnalyzedSubsystem,
    ...
    fn generate_request_constructor(
        &self,
        routine: &AnalyzedRoutine,
        struct_name: &str,
    ) -> Result<String, CodegenError> {
            ...
            if field.is_type_descriptor {
                ...
                if let Some(df) = data_field {
                    ...
                    if df.c_type.contains("mach_port") || df.c_type == "PortName" {
                        ...
                        output.push_str(&format!(
                            "                {}: MachMsgType::integer_64(1),\n",
                            field.name
                        ));
                    } else {
                        output.push_str(&format!(
                            "                {}: MachMsgType::integer_32(1),\n",
                            field.name
                        ));
                    }
                } else {
    ...
    fn generate_client_stub(&self, routine: &AnalyzedRoutine) -> Result<String, CodegenError> {
            ...
            match arg.direction {
                Direction::In | Direction::InOut | Direction::RequestPort => {
                    let param_type = if is_array {
                        ...
                        let field = routine.request_layout.fields.iter().find(|f| {
                            f.name == arg.name && !f.is_type_descriptor && !f.is_count_field
                        });
                ...
                Direction::Out => {
                    if let Some(reply_layout) = &routine.reply_layout {
                        let field = reply_layout.fields.iter().find(|f| {
                            f.name == arg.name && !f.is_type_descriptor && !f.is_count_field
                        });
        ...
        if routine.is_simple {
            ...
            output
                .push_str("            .with_remote_port(port, MACH_MSG_TYPE_COPY_SEND as u32)\n");
            ...
            output.push_str(&format!(
                "        let mut reply: {} = unsafe {{ std::mem::zeroed() }};\n\n",
                reply_struct
            ));
            ...
            if !returns.is_empty() {
                ...
                if returns.len() == 1 {
                    ...
                    let return_exprs: Vec<String> = returns
                        .iter()
            ...
            if routine.is_simple {
                ...
                if returns.is_empty() {
                    ...
                    let return_types: Vec<String> =
                        returns.iter().map(|(_, ty)| ty.clone()).collect();
                    output.push_str(&format!("({}), IpcError> {{\n", return_types.join(", ")));
                }
            }
    ...
    fn generate_server_traits(&self, analyzed: &AnalyzedSubsystem) -> Result<String, CodegenError> {
        ...
        for routine in &analyzed.routines {
            ...
            if self.async_api {
                output.push_str(&format!(
                    "        async fn {}(&self) -> Result<(), IpcError>;\n",
                    routine.name
                ));
            } else {
                output.push_str(&format!(
                    "        fn {}(&self) -> Result<(), IpcError>;\n",
                    routine.name
                ));
            }
        }
