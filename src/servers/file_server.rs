...
impl FileServer {
    ...
    fn create_initial_filesystem(&self) {
        ...
        // Root directory
        files.insert("/".to_string(), FileEntry {
            path: "/".to_string(),
            data: Vec::new(),
            stat: FileStat {
                size: 0,
        ...
        // /dev directory
        files.insert("/dev".to_string(), FileEntry {
            path: "/dev".to_string(),
            data: Vec::new(),
            stat: FileStat {
                size: 0,
        ...
        // /dev/null
        files.insert("/dev/null".to_string(), FileEntry {
        ...
        files.insert("/hello.txt".to_string(), FileEntry {
    ...
    pub fn file_open(&self, path: String, flags: u32, requesting_task: TaskId) -> Result<i32, i32> {
        ...
        if let Some(file_entry) = files.get(&path) {
            if file_entry.is_directory && (flags & 0o3) != 0 { // O_RDONLY = 0
    ...
    pub fn file_read(&self, fd: i32, count: usize, requesting_task: TaskId) -> Result<(Vec<u8>, isize), i32> {
        let mut descriptors = self.file_descriptors.lock();
        let files = self.files.lock();
    ...
    pub fn file_write(&self, fd: i32, data: Vec<u8>, requesting_task: TaskId) -> Result<isize, i32> {
        let mut descriptors = self.file_descriptors.lock();
        let mut files = self.files.lock();
...
pub fn file_server() -> &'static FileServer {
    unsafe {
        (*core::ptr::addr_of!(FILE_SERVER)).as_ref().expect("File Server not initialized")
    }
}