...
impl MemoryMapManager {
    ...
    pub fn print_memory_map(&self) {
        ...
        crate::boot::serial_println("");
        crate::boot::serial_println(&format_memory_summary(self.total_memory, self.usable_memory));
    }
...
fn format_memory_entry(index: usize, entry: &MemoryMapEntry, type_str: &str) -> heapless::String<128> {
    ...
    result.push_str(" - 0x").unwrap();
    result.push_str(&format_hex(entry.start_addr + entry.size - 1)).unwrap();
