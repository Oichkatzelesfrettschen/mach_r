...
impl Gic400Driver {
    ...
    fn read_cpu_reg(&self, offset: usize) -> u32 {
        unsafe {
            read_volatile((self.cpu_interface_base + offset) as *const u32)
        }
    }
    ...
    pub fn set_interrupt_trigger(&self, irq: u32, trigger: InterruptTrigger) -> Result<(), DriverError> {
        if !self.initialized || irq >= self.max_interrupts {
            return Err(DriverError::InvalidOperation);
        }
    ...
    pub fn register_handler(&mut self, irq: u32, handler: InterruptHandler) -> Result<(), DriverError> {
        if !self.initialized || irq >= self.max_interrupts {
            return Err(DriverError::InvalidOperation);
        }
    ...
    pub fn handle_interrupt(&self) {
        ...
        if let Some(handler_opt) = self.handlers.get(irq as usize) {
...
impl DeviceDriver for Gic400Driver {
    ...
    fn bind(&mut self, device: &DeviceInfo) -> Result<(), DriverError> {
        ...
        if device.addresses.len() >= 2 {
            if let (Some(dist_addr), Some(cpu_addr)) = (device.addresses.get(0), device.addresses.get(1)) {
                if let (DeviceAddress::Mmio { base: dist_base, size: _ }, 
                        DeviceAddress::Mmio { base: cpu_base, size: _ }) = (dist_addr, cpu_addr) {
