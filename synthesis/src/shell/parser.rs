...
impl Token {
    ...
    pub fn new(token_type: TokenType, value: &str) -> Result<Self, &'static str> {
        ...
        token_value.push_str(value).map_err(|_| "Token value too long")?;
        ...
        match ch {
            ...
            ' ' | '\t' | '\n' => {
                if in_quotes {
                    ...
                    tokens.push(Token::new(TokenType::Word, &current_word)?)
            ...
            '|' => {
                if in_quotes {
                    ...
                    tokens.push(Token::new(TokenType::Pipe, "|")?)
                        .map_err(|_| "Too many tokens")?;
                }
            }
            ...
            '&' => {
                if in_quotes {
                    ...
                    tokens.push(Token::new(TokenType::Background, "&")?)
                        .map_err(|_| "Too many tokens")?;
                }
            }
            ...
            '<' | '>' => {
                if in_quotes {
                    ...
                    if ch == '>' && chars.peek() == Some(&'>') {
                        ...
                        tokens.push(Token::new(TokenType::Redirect, ">>")?)
                            .map_err(|_| "Too many tokens")?;
                    } else {
        ...
        match token.token_type {
            TokenType::Word => {
                if let Some(redirect_type) = expect_filename {
                    ...
                    filename.push_str(&token.value).map_err(|_| "Filename too long")?;
                    ...
                    command.program.push_str(&token.value)
                        .map_err(|_| "Program name too long")?;
                } else {
                    // Subsequent words are arguments
                    let mut arg = String::new();
                    arg.push_str(&token.value).map_err(|_| "Argument too long")?;
...
pub fn expand_variables(input: &str, get_env: impl Fn(&str) -> Option<&str>) -> Result<String<512>, &'static str> {
    ...
    while let Some(ch) = chars.next() {
        if ch == '$' && chars.peek().is_some() {
            ...
            if let Some(value) = get_env(&var_name) {
                result.push_str(value).map_err(|_| "Expanded string too long")?;
            }
