            ...
            if d.len() >= 4 {
                ...
                if code == 0 {
                    if d.len() >= 12 {
                        ...
                        return Ok(u64::from_le_bytes(pb));
                    } else {
                        return Err(-1);
                    }
                } else {
...
pub fn dispatch<T: NameService>(svc: &T, msg: &Message) -> Option<Message> {
    ...
    match msg_id {
        PAGE_REQUEST_ID => {
            ...
            let mut ob = [0u8; 8];
            ob.copy_from_slice(&data[off..off + 8]);
            off += 8;
            let object_id = u64::from_le_bytes(ob);
            let mut offb = [0u8; 8];
            offb.copy_from_slice(&data[off..off + 8]);
            off += 8;
            let offset = u64::from_le_bytes(offb);
            let mut sb = [0u8; 4];
            sb.copy_from_slice(&data[off..off + 4]);
            off += 4;
            let size = u32::from_le_bytes(sb);
            let mut pb = [0u8; 4];
            pb.copy_from_slice(&data[off..off + 4]);
            off += 4;
