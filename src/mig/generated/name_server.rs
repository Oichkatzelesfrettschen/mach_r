...
impl NameClient {
    pub fn new(server: Arc<Port>) -> Self {
        Self { server }
    }
    ...
    pub fn register_call(&self, name: &str, port: PortId, reply: &Arc<Port>) -> i32 {
        ...
        if let Some(rep) = reply.receive() {
            let d = rep.data();
            if d.len() >= 4 {
                let mut rc = [0; 4];
                rc.copy_from_slice(&d[0..4]);
                return i32::from_le_bytes(rc);
            }
        }
        -1
    }
        ...
        if let Some(rep) = reply.receive() {
            ...
            if d.len() >= 4 {
                ...
                if code == 0 {
                    if d.len() >= 12 {
                        let mut pb = [0u8; 8];
                        pb.copy_from_slice(&d[4..12]);
                        return Ok(PortId(u64::from_le_bytes(pb)));
                    } else {
                        return Err(-1);
                    }
                } else {
...
pub fn dispatch<T: NameService>(svc: &T, msg: &Message) -> Option<Message> {
    ...
    match msg_id {
        REGISTER_ID => {
            ...
            let mut lb = [0u8; 4];
            lb.copy_from_slice(&data[off..off + 4]);
            off += 4;
            let nlen = u32::from_le_bytes(lb) as usize;
            if data.len() < off + nlen + 8 {
                return None;
            }
            let name = core::str::from_utf8(&data[off..off + nlen]).ok()?;
            off += nlen;
            let mut pb = [0u8; 8];
            pb.copy_from_slice(&data[off..off + 8]);
            off += 8;
