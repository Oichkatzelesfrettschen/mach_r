...
use crate::port::{Port, PortRights};
...
impl NameServer {
    ...
    pub fn register(
        ...
        port_names.entry(port_id).or_default().push(name);
...
pub fn name_server() -> &'static NameServer {
    unsafe {
        (*core::ptr::addr_of!(NAME_SERVER))
            .as_ref()
            .expect("Name Server not initialized")
    }
}
...
mod tests {
    ...
    fn mig_style_register_lookup_roundtrip() {
        ...
        let name = "svc";
        let req = Req {
            op: NameServerOp::Register,
            name_len: name.len() as u32,
        };
        // Build OutOfLine message data
        let mut data = alloc::vec::Vec::new();
        let req_bytes = unsafe {
            core::slice::from_raw_parts(&req as *const _ as *const u8, core::mem::size_of::<Req>())
        };
