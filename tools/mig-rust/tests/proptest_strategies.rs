...
pub fn routine() -> impl Strategy<Value = String> {
    (
        ...
        let keyword = if is_simple { "simpleroutine" } else { "routine" };
...
pub fn array_type() -> impl Strategy<Value = String> {
    (array_size(), builtin_type_name())
        .prop_map(|(size, elem_type)| {
            format!("array[{}] of {}", size, elem_type)
        })
}
...
pub fn array_bounds() -> impl Strategy<Value = (u32, u32)> {
    (1u32..100u32, 100u32..10000u32)
        .prop_filter("max must be > min", |(min, max)| max > min)
}
...
pub fn complete_defs_file() -> impl Strategy<Value = String> {
    ...
    ).prop_map(|(name, base, types, routines, server_prefix, user_prefix)| {
