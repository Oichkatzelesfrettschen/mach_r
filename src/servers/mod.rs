...
pub fn poll_once() {
    // Name Server
    let _ = core::hint::black_box(unsafe { (*core::ptr::addr_of!(name_server::NAME_SERVER)).as_ref() })
        .map(|ns| ns.poll_once());
    // VM Server
    let _ = core::hint::black_box(unsafe { (*core::ptr::addr_of!(vm_server::VM_SERVER)).as_ref() })
        .map(|vm| vm.poll_once());
    // Pager Server
    let _ = core::hint::black_box(unsafe { (*core::ptr::addr_of!(pager_server::PAGER_SERVER)).as_ref() })
        .map(|pg| pg.poll_once());
}
