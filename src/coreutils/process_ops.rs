...
pub fn ps(args: &[&str]) -> Result<CommandResult, &'static str> {
    ...
    // Print header
    if show_users {
        result.add_output("  PID  USER     %CPU %MEM    VSZ   RSS TTY      STAT START   TIME COMMAND\n")?;
    } else {
    ...
    if show_users {
        result.add_output("    1 root      0.0  0.1   1234   567 ?        S    00:00   0:00 init\n")?;
        result.add_output("    2 root      0.0  0.0      0     0 ?        S    00:00   0:00 [kthreadd]\n")?;
        result.add_output("   42 root      0.1  0.2   2345  1123 tty1     S    00:01   0:01 mach_r_shell\n")?;
    } else {
