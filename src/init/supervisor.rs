...
impl ProcessSupervisor {
    ...
    pub fn start_process(
        ...
        self.processes
            .push(process_info)
            .map_err(|_| "Too many processes")?;
        self.pid_index
            .insert(pid, index)
            .map_err(|_| "PID index full")?;
    ...
    pub fn wait_process(&mut self, pid: u32) -> Result<i32, &'static str> {
        ...
        match process.state {
            ...
            ProcessState::Exited | ProcessState::Killed => Ok(process.exit_code.unwrap_or(-1)),
    ...
    pub fn reap_children(&mut self) -> Result<Vec<(u32, i32), 32>, &'static str> {
        ...
        for i in 0..self.processes.len() {
            ...
            if process.state == ProcessState::Running {
                ...
                if process.pid.is_multiple_of(10) && get_timestamp().is_multiple_of(1000) {
                    process.state = ProcessState::Exited;
                    process.exit_code = Some(0);
                    reaped
                        .push((process.pid, 0))
                        .map_err(|_| "Too many reaped processes")?;
                }
            }
        }
    ...
    fn find_process_index(&self, pid: u32) -> Result<usize, &'static str> {
        self.pid_index.get(&pid).copied().ok_or("Process not found")
    }
