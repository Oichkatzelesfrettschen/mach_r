...
//! # Safety
//!
//! This module performs memory-mapped I/O (MMIO) to hardware registers.
//! The base addresses must point to valid hardware register regions.
...
pub mod pl011 {
    ...
    impl SerialPort for Pl011Uart {
        fn init(&mut self) {
            ...
            unsafe {
                // Disable UART
                core::ptr::write_volatile(&mut regs.cr, 0);

                // Set baud rate to 115200
                // Assuming 48MHz clock
                core::ptr::write_volatile(&mut regs.ibrd, 26); // 48000000 / (16 * 115200) = 26.042
                core::ptr::write_volatile(&mut regs.fbrd, 3); // 0.042 * 64 = 2.688 ≈ 3

                // Enable FIFO, 8-bit data, no parity, 1 stop bit
                core::ptr::write_volatile(&mut regs.lcr_h, (1 << 4) | (3 << 5));

                // Enable UART, TX, and RX
                core::ptr::write_volatile(&mut regs.cr, (1 << 0) | (1 << 8) | (1 << 9));
            }
        }
        ...
        fn write_byte(&mut self, byte: u8) {
            ...
            // Wait until TX FIFO not full (using volatile read for MMIO)
            while unsafe { core::ptr::read_volatile(&regs.fr) } & (1 << 5) != 0 {
                core::hint::spin_loop();
            }

            // Write byte (volatile write for MMIO)
            unsafe { core::ptr::write_volatile(&mut regs.dr, byte as u32) };
        }

        fn read_byte(&mut self) -> Option<u8> {
            let regs = self.regs();

            // Check if RX FIFO empty (volatile read for MMIO)
            if unsafe { core::ptr::read_volatile(&regs.fr) } & (1 << 4) != 0 {
                None
            } else {
                // Volatile read for data register (MMIO)
                Some((unsafe { core::ptr::read_volatile(&regs.dr) } & 0xFF) as u8)
            }
        }

        fn has_data(&self) -> bool {
            let regs = self.regs();
            // Volatile read for flag register (MMIO)
            (unsafe { core::ptr::read_volatile(&regs.fr as *const u32) } & (1 << 4)) == 0
        }
    }
}
